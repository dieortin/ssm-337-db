...
router.get('', function (req, res, next) {
    database.getAllMembers( function (err, members) {
...
/*router.put('/:name', function (req, res, next) {
    database.addMember(req.params.name, function (err) {
        if (err) return next(err);
        res.status(201).send();
    });
})
...
router.put('', function (req, res, next) {
    database.addMember(req.query.name, req.query.surnames, req.query.birthDate, function (err) {
        if (err) return next(err);
        res.status(201).send();
    });
})
...
router.delete('/:memberId', function (req, res, next) {
    database.removeMember(req.params.memberId, function (err) {
        if (err) return next(err);
        res.status(204).send();
    })
})
